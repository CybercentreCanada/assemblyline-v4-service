...
class TestTagReducer:
    @staticmethod
    def test_constants():
        from assemblyline_v4_service.common.tag_reducer import NUMBER_REGEX, ALPHA_REGEX, ALPHANUM_REGEX, BASE64_REGEX, \
            DO_NOT_REDUCE
    ...
    @staticmethod
    @pytest.mark.parametrize("val, correct_placeholder",
        [
            ("", "${UNKNOWN_TYPE}"),
            ("2", "${NUMBER}"),
            ("a", "${ALPHA}"),
    ...
    @staticmethod
    @pytest.mark.parametrize("uri_parts, correct_uri",
        [
            ({"path": ["path"], "query": {"query": "answer"}, "scheme": "scheme", "netloc": "domain", "params": "params", "fragment": "fragment"}, "scheme://domain/path;params?query=a#fragment"),
            ({"path": ["path"], "query": "", "scheme": "scheme", "netloc": "domain", "params": "params", "fragment": "fragment"}, "scheme://domain/path;params#fragment"),
        ]
    )
    def test_get_placeholder(uri_parts, correct_uri):
    ...
    @staticmethod
    @pytest.mark.parametrize("uris, correct_tags",
        [
            (
                [
            ...
            (
                [
                    "https://en.wikipedia.org/wiki/somelink0",
                    "https://en.wikipedia.org/wiki/somelink1",
                    "https://en.wikipedia.org/wiki/somelink2",
                    "https://en.wikipedia.org/wiki/somelink3",
                ],
